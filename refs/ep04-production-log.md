...
| 7 | 본편 렌더 | video-producer | 🔄(**본렌더 보류**) | `video/output/04_v2/episode.mp4` | **480p 초벌 완료**(EXIT=0, 본선 인수): BGM 첫 실전 통과(차 19.0 LU·믹스 -16.1 LUFS), **길이 불변식 205.53s 일치**, 프레임 표본 9장 육안(법무 표기 실재·seg11 수치 정확). 파노라마 리샘플 병목(4.64s/프레임)은 `_render_source` 축소 훅으로 수리(9f57dde). **결함 후보 1건: seg13 법칙 문구 꼬리가 그림자 패널에 3프레임 가림("낳" 반 잘림, 총감독 f185s.png 육안 확정)** — 그림자는 비사진·비구역이라 layout-audit 사각. 판정·수리·부분 재렌더 후에만 1080p 개시(지시 완료). 일반 처방(그림자류 구역 등록) 검토 포함 |
...
7. **불변식 승격 후보(음성)**: "자/초 > 9.8 = 꼬리 결손 예고 지표" — 4편 take1에서 이상치 3건(seg9 10.72·seg13 11.18·seg14 9.81) 중 2건이 실제 결손으로 확정된 실측 근거. STT 전 사전 적발 지표로 검수 파이프에 상시 편입 검토.
