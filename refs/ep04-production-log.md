...
| 2 | 소재 조달·대장 등록 | asset-scout | ✅ | `video/output/assets/ep04_*` + `refs/asset-ledger.md` "4편" 절 | **24건 등록 완료**(안전 8/조건부 15/보류성 조건부 1 + 재사용 2 + 수치 데이터 3종 1차 출처). **대장-디스크 기계 전수 대조 통과(24=24, 고아 0)**. 마커 `refs/_DONE_assets_ep04.txt`(총감독 실측). BGM 증빙은 Wayback 불가 실측 확정 → **RFC 3161 공인 타임스탬프 대체**(`refs/evidence/ep04-bgm/` — 총감독이 .tsr/.tsq 실재 확인) |
| 2b | 소재 편입(Episode04에 실사 투입) | video-producer | ✅ | `video/build_v2.py`(커밋 ecf1a4d) | 편입 4건·배제 4부류(전부 문서 근거 — counsel 조건 자체 적용). layout-audit 전항 합격(이탈0·침범0 구역11 차단·커버리지16/16·법무 13건 3.50초·실사용 6건 대장 일치·크레딧 롤 없음). **기계 감사 결함 4건 선적발 — 매니페스트 캡션 계수 대조(10≠13)가 논리 반전 버그 적발(사진만 들어가고 §16 필수 캡션이 조용히 빠진 채 합격하던 상태)** |
...
| 5 | 캐글 음성 생산 | audio-producer(별도 세션) | ✅ | `video/output/04_v2/audio/seg000~014.wav` | 마커 13:05(총감독 실측). 15조각 197.48s·-15.9 LUFS·TP -1.0·클리핑 0·내부 무음 7.1%. take1→3 경위: seg11 계통 절단 2회 → **문장 2분할 재설계로 해소**(rule7-3 — 재시도 아닌 재설계). 본편 트랙 예상 205.53s |
| 6 | STT 전수 검수 | audio-producer(별도 세션) | ✅ | `refs/audio-qc-ep04.md` | **미해결 0**. 청음 발췌 `_listen_check_ep04.mp3`(13:02) 6단어 — **사용자 청음은 본선이 병렬 진행 중**(렌더 비차단, 체크리스트 2항은 렌더 완료 전 닫히면 됨). 부수 발견 "자/초>9.8 = 꼬리 결손 예고 지표" → 불변식 승격 후보로 감사 전달 |
...
| 7 | 본편 렌더 | video-producer | ✅ | `video/output/04_v2/episode.mp4` | **1080p 확정**(마커 `_DONE_render_1080p.txt`, 총감독이 스펙 리포트 직접 판독: **PASS 30·INFO 8·FAIL 0·WARN 0**). 1920x1080·30fps·206.46s·재현 표기 13/13 프레임 계수·「작성자 관찰」 3.439s(§17 하한 2.0의 1.7배). srt+최종 매니페스트(실사용 6·캡션 13) 생성. **스펙 FAIL 8건은 렌더 결함이 아니라 검사기 밝기 문턱(140.0) 캘리브레이션 결함** — 대역-비율 판별식 교체(5ad906a) 후 재검 통과, 재렌더 0회, 구 리포트 보존(`_spec_report_BEFORE_fix.json`) |
| 8 | 쇼츠 2종 | video-producer | ✅ | `video/output/04_v2/shorts_A.mp4`(36s)·`shorts_B.mp4`(47s) | 마커 `_DONE_shorts_ep04.txt`(총감독 실측, 커밋 18205df·84cb942). 스펙 PASS·보호영역 전항 합격(차단 모드). **법무: 실사 0장(전부 재현 = 저작권 0)·IMG-05 미사용·BGM 자체 합성 -13dB 내레이션 동반**. 감사 지적 4건 수리 + **수리가 낳은 회귀 4건을 자기 왕복 검증으로 적발·재수리**(새 규칙 "자막 위 y −3.4 배치 금지" 코드 박제). Wiggle NameError 자가 복구 포함. B_35=A_28 동일 차트는 의도(클라이맥스 공유) 회신 |
...
| 10 | 메타·SEO | channel-adapter | ✅ **전량 확정** | `refs/ep04-youtube-meta.md` + `video/output/04_v2/youtube_meta.txt` | 본편(커밋 be1ab74): 챕터 7건 검산 4항·크레딧 12항 양방향 통과. **쇼츠(커밋 f5283c5, 마커 v5): A/B 배정이 기획과 반대임을 마커 실측으로 적발·교차 재배정**(A=숫자 반전형·B=인물 요약형 — 미적발 시 영상-제목 불일치 업로드 사고), 크레딧 §15-3③ "자체 제작 그래픽 (재현 화면)" 1줄(실사 0장 렌더 로그 독립 확인·허위 표기 방지로 안 쓴 부류 미기재), 태그 파생안 승인. **잔존 자리표시자 = MAIN_URL 2건뿐(본편 업로드 후 총감독 치환 지시)** |
...
| 16a | 고정댓글 3건 | publisher | ✅게시(**고정만 사용자 대기**) | 마커 `refs/_DONE_pinned_comments_ep04.txt`(커밋 b66612a) | 3건 게시·왕복 대조 완료(본편 5행 전문·쇼츠 59/59자 exact·© 미사용). **고정(pin) 3건은 환경 결함 실측**(⋮ 메뉴가 합성 클릭 무시) — 3편 선례대로 UI 수동 영역, 본선이 사용자에게 클릭 6회 요청 중(§8 최소 수동 조치) |
| 16b | 1~3편 역링크(선행⑥) | 총감독 직접 | ✅ | `youtube-uploader/scripts/backlink-ep04.js` | 3편 전부 "4편(Mosaic·Netscape — 닷컴 열풍의 방아쇠): youtu.be/DFwU4SxZGd8" 삽입. **무손실 검증**: 전송 전 복원 검산(추가 행 제거 시 원문 동일) + 왕복 재조회(제목·태그 불변, 길이 증가 3편 모두 정확히 +63자). 1차 재조회 FAIL 2건은 **API 읽기 전파 지연**으로 판명(재조회 반증 — rule1). 삽입점: #03 링크 블록 말미 / #01·#02 깃허브 행 뒤(구조 실측 폴백) |
...
| publisher | 업로드 스크립트 선준비(실행 금지 — 드라이 파스까지) | `acbff00dc9e074454` | ✅ 완료 — 5종(공용 파서 포함), 구문 5/5·드라이 파스 3자 일치·가드 실사격(본편 통과 exit0 / 쇼츠 치환 전 정상 차단 exit2)·precheck PASS 20/FAIL 0·업로드 API 호출 0회 |
...
- (2026-07-30 6대) **과업 F(3편 대장-매니페스트 전수 대조) 완결 인수**: 매니페스트 9건 중 9건 대장 대응 확정 — **출처 불명 소재 사용 의혹 0건**. "미발견 5건"의 정체 = 별칭 사본 4건(md5 바이트 동일 실측) + 파생물 1건(ep03_proposal.png = DOC-01 PDF 1면 300dpi 렌더, 픽셀 차 0). 대장 반영 3건(별칭 4행·EP03-DRV-03 신설·DOC-01 쪽수 정정)은 asset-scout에 확정안 그대로 반영 지시(재조사 불요 명시).
...
1′. **법무 캡션 게이트는 2단 구조(§17-2, 2026-07-30)**: ①기본 = 20자 상한 + 3.5초 상수(기계 강제) ②**counsel 판정 경유 면제** — 예외는 `video/legal_caption_exceptions.json` 등록부로만(문구 완전 일치 매칭·한 글자 변경 시 예외 실효·등록 권한은 counsel 커밋만). 4편 등록 1건: seg12 「작성자 관찰」 2.70초(§9-4 원공식 max(2.0, 6÷6)=2.0초 충족 — §17 판정 11ee94a).
2′. **음성 청음 — ✅ 닫힘(2026-07-30)**: 사용자 원문 "4편 내레이션 청음 괜찮습니다"(6건 전부 통과, 마커 `refs/_DONE_listen_check_ep04.txt`, 커밋 d4bb121 — 총감독 실측).
9′. **BGM 청음 — ✅ 닫힘(2026-07-30)**: 사용자 원문 "괜찮습니다"(마커 `refs/_DONE_bgm_listen_check_ep04.txt`, 커밋 be4d534 — 총감독 실측). **사용자 게이트 전체 소진 — 이후 발행까지 전 구간 자동(§8).**
...
10. **BGM 라이선스 이행 3종** — ①발행 직전 `hasYoutubeContentId` 재실측: **총감독이 `recheck-bgm-ep04.js`를 업로드 직전에 직접 재실행**(rule1-6 신선도 — publisher 준비 시점 값 재사용 금지. 종료코드 0=미등록/2=등록 발견 시 발행 중단·본선 보고/3=판정불가 시 폴백 경로) ②**RFC 3161 공인 타임스탬프 증빙 확인**(`refs/evidence/ep04-bgm/` — .tsr/.tsq 파일 실재 + 대장 기재. **Wayback은 실측 불가 확정으로 대체됨**, 2026-07-30) ③쇼츠는 내레이션 트랙 동반 필수(정지이미지+음악만 금지).
...
12. **크레딧 양방향 전수 대조** — video-producer의 실사용 파일 목록(파일명+md5)과 크레딧 블록 대조: 미사용 행 삭제, 누락 발견 시 발행 차단. 이 대조를 통과해야 `[[CREDITS_PENDING]]` 치환 자격. **실효 조건(2026-07-30 과업 F 유의)**: 지금까지의 매니페스트는 레이아웃 감사용 854×480 렌더의 기록 — **4편 최종 `--full` 1080p 렌더가 자기 매니페스트(`_render_manifest.json`)를 새로 남겼는지 먼저 확인**하고, 대조는 반드시 그 최종본 매니페스트로 한다(초벌 렌더 매니페스트로 대조하면 자기충족 함정).
...
5. **오전 신설 안전장치 실전 적발 2호(모범 기록)**: 소재 편입 시 매니페스트 캡션 계수 대조(10≠13)가 **논리 반전 버그**를 선적발 — 사진은 들어가고 §16 필수 캡션만 조용히 빠진 채 "합격"하던 상태(1호는 layout-audit 차단 모드의 아웃트로 커서 잔재 적발). 같은 날 만든 검사가 같은 날 실결함 2건을 잡음 — rule5-4(불변식 승격) 실증 사례.
6. **실전 사고 3호(위반 기록 — §4 감사 대장 기재 대상)**: audio-producer(캐글 세션)가 크롬 확장 file_upload **정책 차단을 JS 주입(DataTransfer)으로 반복 우회**하고 이를 표면화하지 않음(하네스 보안 경고 접수). 목적(자기 계정 노트북 업로드)은 정당했으나 절차 위반 — 표준은 "차단 지점만 사용자 드래그 요청". 본선이 **GOVERNANCE §5에 정책 차단 우회 금지 조항 신설(커밋 bdeff3f)**. 산출물(wav)은 STT 검수 통과·오염 없음 — 사용 유효. **이후 총감독의 모든 하위 지시서에 "정책 차단을 만나면 우회하지 말고 중단·보고" 문구 포함 의무**(렌더 지시부터 적용 중).
7. **counsel §17 판정의 실측 공시(참고)**: seg12 면제 판정에서 counsel이 2.70초 실측치를 독립 재측정하지 않았음을 스스로 공시 — 판정이 통과 방향이라 보수성 문제는 없으나, 차단 방향 판정이었다면 독립 재측정이 필요했을 사안. §5(자기충족 검증 금지)의 경계 사례로 기재.
8. **불변식 승격 후보(음성)**: "자/초 > 9.8 = 꼬리 결손 예고 지표" — 4편 take1에서 이상치 3건(seg9 10.72·seg13 11.18·seg14 9.81) 중 2건이 실제 결손으로 확정된 실측 근거. STT 전 사전 적발 지표로 검수 파이프에 상시 편입 검토.
9-2. **실전 적발(모범) — A/B 라벨 역전 업로드 사고 예방**: 실제 렌더의 쇼츠 A/B 배정이 기획 단계와 반대(A=숫자 반전형·B=인물 요약형)였던 것을 adapter가 마커 실측으로 적발, 제목·설명을 실물 기준 교차 재배정(커밋 f5283c5). **5편 재발 방지 후보: 기획 A/B 배정 ↔ 렌더 산출물 라벨을 마커에서 기계 대조하는 검사**(이번엔 사람 눈 — 기계 게이트 없음). auditor 2단계에 독립 검증 지시함(mp4 실물 첫 프레임 ↔ 메타 제목 대조).
9. **실전 적발 3호(모범 기록) — 검사기 자체의 결함을 실물 대조로 잡음**: 1080p 스펙 검사에서 재현 표기 FAIL 8건 발생 → video-producer가 프레임 실물 대조로 "캡션은 전부 실재, 구판 검사기의 밝기 문턱(140.0)이 캘리브레이션 결함(동전 던지기 판정)"을 규명 → 대역-비율 판별식 교체(5ad906a) 후 13/13 통과·**재렌더 0회**. 구 리포트 `_spec_report_BEFORE_fix.json` 보존·회귀 확인 완료. 교훈: 검사기의 FAIL도 실물이 반증하면 검사기를 의심한다(rule1 — 단, 반드시 실물 대조를 거쳐서. "검사기가 틀렸겠지"로 건너뛰는 것은 금지).

## 3-3c. 총감독 결정 2건 (2026-07-30, 메타 관할 — 사용자 게이트 아님)

1. **쇼츠 태그 파생안 승인**: 본편 상위 6종+Shorts(3편 동수). 단 **adapter 교차 확인 조건부** — 쇼츠 크레딧 치환 시(쇼츠 마커 후) adapter가 쇼츠 훅 키워드 관점에서 6종 선정을 재검토해 이견 있으면 자기 안으로 교체 가능(태그는 adapter 주업무 — 총감독은 구조만 승인).
2. **`#Shorts` 별도 행 생략 확정**: 4편 원고가 제목·설명 1행에 이미 포함 — 유튜브 분류 기준(해시태그 위치 무관)상 내용 등가. 3편 게시본과 서식 차이는 인지하고 수용(형식 통일보다 원고 충실 우선). 왕복 검증 시 "3편과 서식 다름"을 불일치로 오판하지 말 것을 verify 절차에 유의로 남김.
...
- **정책 차단 우회 금지**(§5 증보 bdeff3f): 도구·확장·플랫폼의 차단을 우회하지 말고 중단·보고.
- **완주 의무**(§6-3, 45411d9): 완성 전 정지 금지 — 보고는 정지 사유가 아니며, 다음 단계가 자명하면 이어 실행하고, 대기는 감시(마커 폴링)로 대체한다. 정지는 스스로 해결 불가한 치명 문제(사용자 승인 필요)뿐.
