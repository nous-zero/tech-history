...
| 10 | 메타·SEO | channel-adapter | ✅(본편 확정) | `refs/ep04-youtube-meta.md` + `video/output/04_v2/youtube_meta.txt` | **본편 메타 전량 확정 — 업로드 투입 가능**(커밋 be1ab74, 마커 v4): 챕터 7건 치환+검산 4항 통과(전 장 ≥10초 — 3편 7.5초 사고 재발 없음, mp4 길이 mvhd 원자 직접 파싱 독립 실측), **크레딧 12항 양방향 대조 통과·확정 전환**(순6/6·역6/6, PD/CC0·BGM 자발 공시, 미사용 슬롯 제거). 잔존 자리표시자 = 정당 2종뿐(쇼츠 CREDITS_PENDING — 쇼츠 매니페스트 대기·MAIN_URL — 본편 업로드 후) |
