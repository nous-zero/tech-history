...
| 2b | 소재 편입(Episode04에 실사 투입) | video-producer | 🔄 | `video/build_v2.py` | **전면 해제(2026-07-30)** — counsel §16 캡션 완성형 6종 + 구속 2건(엔딩 크레딧 롤 금지·manifest 전수 포함) 전달. 편입→layout-audit 재실행→wav 폴링 |
...
| 5 | 캐글 음성 생산 | audio-producer(별도 세션) | 🔄 take2 | `video/output/04_v2/audio/seg000~014.wav` | **임계 경로(음성 단독)**. take1 15조각 회수(24kHz 모노, 201.88s) → STT 전수: **불합격 3(seg9·11·13, 전부 '마지막 문장 미생성' — 3편과 동일 부류)**, 물리 검증(꼬리 크롭 2모델 재판독)으로 결손 확정 → 11:57 take2 재생산 개시(ONLY=[9,11,13], 노트북 V2). take1 원본 audio_raw/ 보존. 다듬기 예측 201.88→186.42s(-7.7%) |
