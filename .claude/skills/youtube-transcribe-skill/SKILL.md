...
- **Privacy note (local patch)**: try WITHOUT browser cookies first. Only if the video
  requires sign-in (age-restricted/members-only error), ask the user for permission to
  add `--cookies-from-browser=<their browser>` and retry.

### 2.3 Download Subtitles

```bash
yt-dlp --write-auto-sub --write-sub --sub-lang ko,en --skip-download --output "<Video Title>.%(ext)s" "[VIDEO_URL]"
```

- Default subtitle languages are `ko,en` for this project (Korean tech-history channel);
  add other languages when the user asks.
