...
print(
    ...
    "저장소 구조(A안, 2026-07-31): **origin = 비공개 본진 tech-history-ops**(일상 커밋·푸시 기본 목적지), public = 공개 tech-history. "
    "자동 격리(§10-2): 공개는 `python tools/sync_public.py`(허용 목록 posts/·LICENSE·README만)로만 나가고, 공개 remote 직접 푸시는 게이트가 차단한다. "
    "분류 불명 = 비공개(fail-closed) — 민감물 취급은 승인 불요, 승인은 허용 목록 변경·비가역 작업·가동 인프라 변경에만. "
    "보안 감시(§10-3): 주간 감사·보안 스캔 시 `python tools/selftest_public_gate.py` 카나리아 필수 실행, auditor가 security-officer 판정을 교차 재실측."
)
sys.exit(0)
